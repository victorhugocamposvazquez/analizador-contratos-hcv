...
export default function DniBatchesBulkTable({ rows }: { rows: DniBatchRow[] }) {
  ...
  async function bulkDelete() {
    ...
    if (
      !window.confirm(
        `¿Eliminar ${n} lote${n !== 1 ? "s" : ""} DNI? Se borran ficheros y datos asociados (irreversible).`
      )
    ) {
      return;
    }
    setBusy(true);
    const r = await fetch("/api/dni-batches/bulk-delete", {
      method: "POST",
        ...
        <table className="w-full text-sm min-w-[640px]">
          <thead className="bg-slate-50 text-slate-600">
            <tr>
              ...
              <th className="text-right px-4 py-2 font-medium">Hechos</th>
              <th className="text-right px-4 py-2 font-medium">En cola</th>
              ...
              return (
                <tr key={b.id} className="border-t hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2">
                    <Link
                      href={`/contracts/dnis/batches/${b.id}`}

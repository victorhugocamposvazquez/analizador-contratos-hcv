...
export default function DniBatchRenameForm({ batchId, initialName }: Props) {
  ...
  async function save() {
    setBusy(true);
    setErr(null);
    const r = await fetch(`/api/dni-batches/${batchId}`, {
      method: "PATCH",
  ...
  return (
    <div className="flex flex-wrap items-end gap-2">
      <label className="flex flex-col gap-1 min-w-[12rem]">
        <span className="text-xs font-medium text-slate-600">Nombre del lote DNI</span>
        <input
          type="text"
          value={value}
          onChange={(e) => setValue(e.target.value)}
          placeholder="Ej. Campaña febrero"
          disabled={busy}
